...
public class cusAccountDAO {
    ...
    public Vector<accountDTO> createAccountList(){
        try{
            ...
            openConnection() ;
            ResultSet rs = statement.executeQuery("SELECT * FROM CustomerAccounts") ;
    ...
    public void addAccount(accountDTO account){
        try{
            openConnection() ;
            String sql = "insert into CustomerAccounts(Username , Password , AccountType) values (? , ?, ?)" ;
    ...
    public void deleteAccount(accountDTO account){
        try{
            openConnection() ;
            String sql = "delete from CustomerAccounts where Username = ?" ;
            PreparedStatement preparedStatement = con.prepareStatement(sql) ;
            preparedStatement.setString(1, account.getUsername()) ;
            preparedStatement.executeUpdate() ;
            preparedStatement.close() ;
    ...
    public void updateAccount(accountDTO account){
        try{
            openConnection() ;
            String sql = "update CustomerAccounts set Password = ? where Username = ?" ;
            PreparedStatement preparedStatement = con.prepareStatement(sql) ;
    ...
    public Vector<accountDTO> searchAccount(String input){
        try{
            openConnection() ;
            String query = "Select * from CustomerAccounts where AccountID like ? or Username like ? " ;
    ...
    public String checkLogin(accountDTO account){
        try{
            openConnection() ;
            String selectUsername = "select * from CustomerAccounts where Username = ?" ;
            String select = "select * from CustomerAccounts where Username = ? and Password = ?" ;
    ...
    public accountDTO getAccountByUsername(String username){
        try{
            openConnection() ;
            String sql = "select * from CustomerAccounts where Username = ?" ;
            PreparedStatement preparedStatement = con.prepareStatement(sql) ;
